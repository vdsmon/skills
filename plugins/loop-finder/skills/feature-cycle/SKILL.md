...
when_to_use: >-
  ...
  full). Do NOT use for ad-hoc one-off features, because the queued-fix
  chain only pays off across multiple cycles. Do NOT use to find or refine
...
- Godot scenario harness + pong (7 cycles, `wall_s` dropped 4.26x F1->F7), proving ground for the queued-fix chain and Absorption Sprint patterns.
