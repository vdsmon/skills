...
Run the script in dry-run mode. The script lives at `scripts/git_cleanup.sh` under this skill's base directory (shown in the skill header as "Base directory for this skill"). Use that absolute path, because the `~/.claude/commands/...` location does not exist for plugin installs:
