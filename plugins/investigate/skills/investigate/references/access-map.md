...
Fill this table in for your own stack. The rows below are the common shapes — swap the tool names and hosts for the ones you actually have.

| System | How you reach it | If blocked |
|---|---|---|
| Code + git history (any repo under your source root, e.g. `~/repos`) | direct (Read, Bash `git`) — the failing code may live in an adjacent repo, not the one you're in | — |
...
1. **Prod orchestrator logs.** Frequently web-UI-only. When the report points at a prod run, you cannot pull the log yourself — ask for a paste of the *full* failing task log (not just the error word; the traceback discriminates the cause).
...
Before digging, confirm the report's identifiers actually resolve against the real platform. Reporters paraphrase: a job name, run-id shape, host, or storage path that doesn't match anything real is a signal the report is imprecise — `git log --all -S '<identifier>'` proves "never existed here" vs "renamed." A close-but-wrong mapping sends the fix to the wrong code path. If identifiers don't resolve, that's a raise (which job/repo/run did you mean?), not a guess.
