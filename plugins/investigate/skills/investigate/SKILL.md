...
Parallelize reachable, independent digs with read-only subagents and synthesize their findings yourself. Subagents dig; they do **not** raise — if one hits a wall, it reports the wall and *you* raise it.

Keep an **evidence trail**: for each system, what you checked and what you found (or that it was clean — clean findings rule things out). This is half the deliverable.
...
1. **Root cause** — the actual cause, each claim tied to specific evidence (this log line, this commit, this row count). If you couldn't reach certainty, say exactly what's still unconfirmed and what evidence would close it — do not round up to a conclusion.
