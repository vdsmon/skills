...
description: Investigate a reported error or incident end to end, digging across every reachable system (any repo, Airflow, cloud and job logs, databases, Jira, Slack) to find the root cause and propose a fix. Use when someone else reports something broken in a running system, or pastes a stack trace, Slack thread, or Jira link. Stops and asks rather than guessing past a source it cannot reach. For a reproducible bug you can run and inspect directly, use systematic-debugging instead.
---
