...
This skill is currently a **skeleton** (build phases 1-4 + 6 + 7-mvp of the implementation plan complete; phase 5 + 7-full + 8-12 still pending). The tracker Protocol, factory, both adapters (Jira + beads), bundle discovery, transactional init wizard, and minimum-viable dispatcher (state.py + validate_workspace.py + dispatch_stage.py) exist; SKILL.md MCP-call refactor, lease lifecycle + TOCTOU snapshot, helper scripts (lint-ticket, branch-ticket, ticket-frontmatter, diff-extract, recover), and memory layer land in later phases.
...
Phases 1-4 + 6 + 7-mvp complete: `plugin.json`, `stage-registry.toml`, `scripts/tracker.py` (Protocol + factory + types), `scripts/tracker_jira.py` (full JiraAdapter), `scripts/tracker_beads.py` (full BeadsAdapter), `scripts/bundle_discover.py` (`.flow-bundle.toml` schema v1 walker + validator), `scripts/init.py` (pure-CLI transactional workspace bootstrap), `scripts/state.py` (atomic per-ticket state.json r/w with flock + rolling backups + quarantine path), `scripts/validate_workspace.py` (HARD GATE schema validator), `scripts/dispatch_stage.py` (state-machine driver — `init` / `next` / `finish` / `status` subcommands; emits handler-descriptor JSON; pending → in_progress → completed | failed lifecycle; validate-workspace re-runs on every `next` as TOCTOU mvp invariant). Lease lifecycle, canonical-snapshot TOCTOU, heartbeat hung-detection, helper scripts (lint-ticket / branch-ticket / ticket-frontmatter / diff-extract / recover), and memory layer = not yet built. Do not call verbs against this skill until phase ≥8.
