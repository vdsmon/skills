...
These don't conflict: be expansive about *when*, restrained about *how*. And you don't have to settle it by taste — Step 5 optimizes the description **empirically** against trigger evals.
...
Note: simple one-step queries ("read this file") often won't trigger any skill regardless of description — the model handles them directly. Trigger evals should be substantive enough that consulting a skill actually helps.
...
Discipline skills (rules the agent must hold under time pressure, sunk cost, exhaustion) need more than a clear statement — smart agents find loopholes. Build these in:
...
After writing any skill you MUST complete its test/verify cycle before moving on. Don't batch-create skills untested "for efficiency" — deploying an untested skill is deploying untested code. Each skill finishes RED-GREEN-REFACTOR before the next one starts.
