...
|------|-----------|-----------------|-------------------|
| **Max** | **1 hour** | 2x base input price | 30-min cron, the only viable interval. 60-min does not work (TTL + jitter = consistent misses, verified test #5b). Intermediate values like 45-min can't be expressed cleanly in cron (only divisors of 60 work: 1,2,3,4,5,6,10,12,15,20,30). |
