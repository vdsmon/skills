...
description: Interrogates a rough objective into a tight, verifiable /goal completion condition, emitted paste-ready. Pass --delegate to run each iteration in a fresh subagent.
...
4. **The forbidden cheat.** Given the gate, what's the cheapest way to satisfy the words without doing the work (delete the test, stub the function, hardcode or regenerate the output it's graded against)? Forbid it explicitly, because the gate is worthless if the loop can edit what it's graded against.
...
If the objective is genuinely compound ("migrate, add OAuth, write docs"), **don't fuse it**, because the evaluator checks one condition and stalls on the slowest sub-part. Recommend splitting into sequential `/goal` runs and sharpen the first.
