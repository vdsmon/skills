...
# pick highest-breaching window across 5h + weekly (float compare)
sel=$(awk -v f="$five" -v s="$seven" -v ft="$FIVE_THRESHOLD" -v st="$SEVEN_THRESHOLD" 'BEGIN{
  fb=(f>=ft); sb=(s>=st);
  if(fb&&sb){ if(f>=s) print "five"; else print "seven" }
  else if(fb) print "five";
  else if(sb) print "seven";
  else print "";
}')
...
# debounce: warn once per (window:reset)
key="$window:$reset_int"
...
${resume_step}
4. Then stop and tell the user you paused and will auto-resume ~${hhmm}."
