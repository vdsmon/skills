...
For the gap cases (a session resumed from disk, where the keepalive hook deliberately doesn't fire, or a flag created after the session started), the PARK instruction adds one step when the flag is present and the reset is more than ~50 minutes out: check `CronList` for a `cc-cache-keepalive` job, and only if missing, create one (anchored cron, same sentinel prompt, same reply contract). Because it reuses the keepalive plugin's opt-in flag, sentinel, and dedup, nothing ever double-schedules and sentinel-matching Stop-hook gates keep working. Without the flag, the guard never schedules a keepalive. The flag's interval override (line 1) is honored when it's under the 1h TTL (seconds round up to minutes); hour/day units, values >= 60m, or invalid values fall back to 30m, since a cadence at or above the TTL can't keep the cache warm.
