...
Current plugins: `skill-polish`, `tokenomics`, `cache-keepalive`, `pre-compact`, `humanize`, `dg`, `converge`. `tokenomics` is analysis + education only; cache warmup lives in `cache-keepalive`.
...
- `allowed-tools` — pre-approved Bash/tool patterns (see `humanize`, `tokenomics`).
...
**Progressive disclosure**: move reference content out of `SKILL.md` into sibling files (see `tokenomics/skills/tokenomics/reference/*.md`). Keep references one level deep — chains of `.md` → `.md` → `.md` cause partial reads. Aim for ≤100 lines in `SKILL.md`.
...
## Hooks (only cache-keepalive uses one)

`plugins/cache-keepalive/.claude-plugin/plugin.json` declares `SessionStart` hook running `hooks/keepalive.sh`. Pattern preserve when adding hooks:

- **Opt-in via flag file in `$HOME`** (`~/.cache-keepalive`). Hook short-circuits with `exit 0` when flag absent — zero output, zero side effects for non-opt-in users.
...
- **Silent-mode rule lives in the cron prompt**, not in any skill — the prompt string emitted by the hook starts with `[Silent cache-keepalive — run Bash tool only. No text output, no acknowledgment, no summary.]`. Model reads that directive and runs Bash silently. Don't duplicate the rule in a SKILL.md.
...
Called from `SKILL.md` via dynamic-context injection: the `` ```! `` block runs `python3 "${CLAUDE_SKILL_DIR}/scripts/token-report.py" $ARGUMENTS` at invocation, so the numbers arrive before the model reads the skill. Can also run directly: `python3 plugins/tokenomics/skills/tokenomics/scripts/token-report.py [--all|<path>]`. Experiment findings live in `plugins/tokenomics/skills/tokenomics/reference/experiments.md` — update the table when running new tests rather than writing new docs.
